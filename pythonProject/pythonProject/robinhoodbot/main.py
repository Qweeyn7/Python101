...
def get_last_crossing(df, days, symbol="", direction=""):
    ...
    if (found != lastIndex):
        if ((direction == "above" and recentDiff) or (direction == "below" and not recentDiff)):
            print(symbol + ": Short SMA crossed" + (" ABOVE " if recentDiff else " BELOW ") + "Long SMA at " + str(
                dates.at[found]) \
                  + ", which was " + str(pd.Timestamp("now", tz='UTC') - dates.at[found]) + " ago",
                  ", price at cross: " + str(prices.at[found]) \
...
def golden_cross(stockTicker, n1, n2, days, direction=""):
    ...
    if (cross) and plot:
        show_plot(price, sma1, sma2, dates, symbol=stockTicker, label1=str(n1) + " day SMA",
...
def buy_holdings(potential_buys, profile_data, holdings_data):
    ...
    ideal_position_size = (safe_division(portfolio_value, len(holdings_data)) + cash / len(potential_buys)) / (
                2 * len(potential_buys))
    ...
    for i in range(0, len(potential_buys)):
        ...
        if (ideal_position_size < stock_price < ideal_position_size * 1.5):
            ...
            print(
                "####### Tried buying shares of " + potential_buys[i] + ", but not enough buying power to do so#######")
            break
